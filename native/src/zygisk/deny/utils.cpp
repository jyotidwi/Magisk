...
static void check_zygote(){
    crawl_procfs([](int pid) -> bool {
        ...
        if (system_server_pid > 0 && is_zygote(pid) && parse_ppid(pid) == 1) {
            new_zygote(pid);
            return true;
        }
...
void proc_monitor() {
    ...
    while (system_server_pid == -1)
        sleep(1);
    system_server_fd = open((string("/proc/") + to_string(system_server_pid)).data(), O_PATH);
    if (access((string("/proc/self/fd/") + to_string(system_server_fd) + "/cmdline").data(), F_OK) != 0) {
        system_server_pid = -1;
        goto start_monitor;
    }
    // now ptrace zygote
